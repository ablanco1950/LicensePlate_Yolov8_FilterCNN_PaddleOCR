...
json_file = open('FilterCNN16Hits500epoch.json', 'r')
#json_file = open('FilterCNN.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
modelCNN = model_from_json(loaded_model_json)
# load weights into new model

modelCNN.load_weights("FilterCNN16Hits500epoch.h5")
#modelCNN.load_weights("FilterCNN.h5")
...
with open( "LicenseResults.txt" ,"w") as  w:
    for i in range (len(imagesComplete)):
            ...
            grayCodFilter = cv2.resize(grayCodFilter, (104,104), interpolation = cv2.INTER_AREA)
            X_test=[]
            #X_testFlat.append(grayCodFilter.flatten())
            X_test.append(grayCodFilter)
            X_test=np.array(X_test)
            
            X_test = X_test.astype("float32") / 255.0
            
                       
            CodFilter=modelCNN.predict(X_test)
           
            #print(CodFilter)
            CodFilter=np.argmax(CodFilter, axis=1)
            print(License + " Cod filter = "+ str(CodFilter))
            

            if CodFilter> 10:
                print("RAREEEEEEE CODE Greather tan 10")
                continue
